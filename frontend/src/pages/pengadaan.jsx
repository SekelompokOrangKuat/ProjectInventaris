import { Box, Grid, Typography, Stack, TextField, Button, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, tableCellClasses, styled, Paper, IconButton, LinearProgress, Select, MenuItem, Autocomplete } from "@mui/material";
...
const Pengadaan = () => {
    ...
    const [dataKodeBarang, setDataKodeBarang] = React.useState([]);
    ...
    const labels = [
        { name: "Nama Barang", controller: setNamaBarang, value: namaBarang, autoComplete: true },
        { name: "Kode Barang", controller: setKodeBarang, value: kodeBarang, autoComplete: true },
        { name: "Nomor Register", controller: setNomorRegister, value: nomorRegister, autoComplete: false },
        { name: "Nama Ruangan", controller: setNamaRuangan, value: namaRuangan, autoComplete: false },
        { name: "Kode Lokasi", controller: setKodeLokasi, value: kodeLokasi, autoComplete: false },
        { name: "Spesifikasi", controller: setSpesifikasiBarang, value: spesifikasiBarang, autoComplete: false },
        { name: "Nama Penanggung Jawab", controller: setNamaPenanggungJawab, value: namaPenanggungJawab, autoComplete: false },
        { name: "Merk/Type", controller: setMerkBarang, value: merkBarang, autoComplete: false },
        { name: "Keterangan", controller: setKeterangan, value: keterangan, autoComplete: false },
        { name: "Upload Gambar", controller: setFotoBarang, value: fotoBarang, autoComplete: false },
        { name: "Jumlah Barang", controller: setJumlahBarang, value: jumlahBarang, autoComplete: false },
        { name: "Ukuran Barang", controller: setUkuranBarang, value: ukuranBarang, autoComplete: false },
        { name: "Bahan Barang", controller: setBahanBarang, value: bahanBarang, autoComplete: false },
        { name: "Tahun Pembelian", controller: setTahunPembelian, value: tahunPembelian, autoComplete: false },
        { name: "Nomor Pabrik", controller: setNomorPabrik, value: nomorPabrik, autoComplete: false },
        { name: "Nomor Rangka", controller: setNomorRangka, value: nomorRangka, autoComplete: false },
        { name: "Nomor Mesin", controller: setNomorMesin, value: nomorMesin, autoComplete: false },
        { name: "Nomor Polisi", controller: setNomorPolisi, value: nomorPolisi, autoComplete: false },
        { name: "Nomor BPKB", controller: setNomorBPKB, value: nomorBPKB, autoComplete: false },
        { name: "Asal Usul", controller: setAsalUsul, value: asalUsul, autoComplete: false },
        { name: "Harga Barang", controller: setHargaBarang, value: hargaBarang, autoComplete: false },
    ];
    ...
    const getBarangInfo = async () => {
        await fetch(
            'https://backend.icygrass-3ea20227.eastasia.azurecontainerapps.io/v1/admin/kb/findAll',
            {
                method: 'GET',
                headers: {
                    'Accept': 'application/json',
            ...
            .then((data) => {
                if (data.response_code === 200) {
                    ...
                    let barangNama = [];
                    for (let i = 0; i < data.data.length; i++) {
                        const kodeBarangJoined = `${data.data[i].golongan}.${data.data[i].bidang}${data.data[i].bidang !== '' ? '.' : ''}${data.data[i].kelompok}${data.data[i].kelompok !== '' ? '.' : ''}${data.data[i].sub_kelompok}${data.data[i].sub_kelompok !== '' ? '.' : ''}${data.data[i].sub_sub_kelompok}`
                        barangKode.push({ label: kodeBarangJoined, id: i });
                        barangNama.push({ label: data.data[i].nama_barang, id: i });
                    }
                    setDataNamaBarang(barangNama);
                    setDataKodeBarang(barangKode);
                } else {
                    console.log('error');
                }
            })
            ...
            <Grid item xs={4}>
                {
                    labels[i].autoComplete
                        ? <Autocomplete
                            disablePortal
                            options={labels[i].name.localeCompare('Nama Barang') === 0 ? dataNamaBarang : dataKodeBarang}
                            renderInput={(params) => <TextField {...params} variant="outlined" label={labels[i].name} />}
                            fullWidth
                            input={labels[i].value}
                            onInputChange={(event, newInput) => labels[i].name.localeCompare('Nama Barang') === 0 ? handleNamaBarangChange(newInput) : handleKodeBarangChange(newInput)}
                        />
